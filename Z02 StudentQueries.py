...
mtcarsDF2.head()
mtcarsDF2 >> arrange(X.cyl, X.mpg)>> head()
mtcarsDF2 >> group_by(X.gear, X.am, X.cyl) >> summarize(meanWT= X.wt.mean(), maxHP = X.hp.max()) >> ungroup()  >> arrange(X.cyl) 

mtcarsDF2 >> group_by(X.carb,X.vs) >> summarize(meanWT = X.wt.mean(), MinMilage= X.mpg.min()) >> ungroup() >> arrange(X.meanWT)
mtcarsDF2 >> mutate(Newmpg = X.mpg*1.2)>> head()
